...
app.get('adjustmentresponsesurvey', function (request,response){
    if (typeof request.param('ARname')!='undefined'){
        client.query('INSERT INTO AR_table (ARname, IDnumber, Surveynumber) VALUES($1, $2, $3)', [request.param('ARname'), request.param('IDnumber'), request.param('Surveynumber')], function(err, result) {
            done();
            if (err) {
                console.error(err); response.send("Error " + err);
    }
